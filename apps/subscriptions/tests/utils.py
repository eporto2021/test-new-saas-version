...
def create_subscription_for_user(subscription_holder: CustomUser, product: ProductMetadata, metered=False):
...
def create_subscription(customer, product):
    subscription = Subscription.objects.create(
        ...
        current_period_start=timezone.now() - relativedelta(months=1),
...
def get_mock_request(user):
    """
    Simulates a request object which can be passed directly to a mock view, with a logged-in user.
    """
