...
def create_stripe_checkout_session(
    ...
    if subscription_holder.customer:
        # Always ensure customer email matches user email in both local DB and Stripe
        if subscription_holder.customer.email != user.email:
            # Update local customer
            subscription_holder.customer.email = user.email
            subscription_holder.customer.save()
            
            # Update customer in Stripe
            stripe.Customer.modify(
                subscription_holder.customer.id,
                email=user.email,
            )
            
            # Sync local customer with updated Stripe data
            stripe_customer = stripe.Customer.retrieve(subscription_holder.customer.id)
            subscription_holder.customer.sync_from_stripe_data(stripe_customer)
        
        customer_kwargs["customer"] = subscription_holder.customer.id
        # Only add customer_update when we have a customer
        customer_kwargs["customer_update"] = {
            "address": "auto",
            "name": "auto",
        }
    else:
